...
data_merged %>%
    group_by(year) %>%
    ggplot() +
    geom_line(mapping = aes(x = year, y = life_expectancy, color = continent, goup = country), 
                na.rm = TRUE) +
    my_theme()
...
data_merged %>%
    ggplot() +
      geom_point(mapping = aes(x = fossil_fuel_consumption/population, y = renewables_consumption/population, color = continent), 
                 na.rm = TRUE) +
    my_theme()
...
data_merged %>%
    filter(year == 2020) %>%
    ggplot() +
      geom_point(mapping = aes(x = life_expectancy, y = primary_energy_cons_per_capita, color = continent), 
                 na.rm = TRUE) +
    my_theme()
...
data_merged %>%
    #filter(year == 2020) %>%
    # summarise(primary_energy_consumption = sum(primary_energy_consumption),
    #           population = sum(population)) %>%
    ggplot() +
      geom_line(mapping = aes(x = year, y = primary_energy_cons_per_capita, color = continent, group = country), 
                 na.rm = TRUE) +
    xlim(1960,2025) +
    my_theme()
