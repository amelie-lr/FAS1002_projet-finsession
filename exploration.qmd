...
#     filter(year == 2020) %>%
#     ggplot() +
#       geom_line(mapping = aes(x = country, y = life_expectancy), 
#                 na.rm = TRUE) +
#     my_theme()
...
Consommation d'énergie fossile vs renouvelable

```{r}

data_energy_cons_clean %>%
    ggplot() +
      geom_point(mapping = aes(x = renewables_consumption, y = fossil_fuel_consumption, color = continent), 
                 na.rm = TRUE) +
    my_theme()
